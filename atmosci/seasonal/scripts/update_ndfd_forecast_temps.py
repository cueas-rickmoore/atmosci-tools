...
if not os.path.exists(filepath_template % ('001-003', 'mint')):
    ...
    file_template = ndfd_factory.forecastGribFilepath(ndfd,date,'%s','%s')
...
for temp_var in ('maxt','mint'):
    ...
    for time_span in ('001-003','004-007'):
        grib_filepath = file_template % (time_span, temp_var)
        ...
        for message_num in range(len(grib)):
            ...
            if fcast_time > 158: # forecastTime is MINUTES
                fcast_time = analysis_date + timedelta(minutes=fcast_time)
            else: # forecastTime is hoors
                fcast_time = analysis_date + timedelta(hours=fcast_time)
...
manager = \
    temps_factory.tempextsFileManager(target_year, source, region, mode='a')
