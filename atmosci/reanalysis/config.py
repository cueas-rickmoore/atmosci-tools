...
BY_HOURS_DIRPATH = \
    ('grid','%(region)s','%(analysis)s','%(num_hours)dhours','%(target_date)s')
BY_YEAR_DIRPATH = \
    ('grid','%(region)s','%(analysis)s','%(target_year)s','%(target_month)s')
...
CONFIG.sources.reanalysis = {
    'source_priority':('rtma','urma'), # higher index = higher priority
    'tag':'Reanalysis',
    ...
    'grib': {
        ...
        'variable_names':('APCP','CEIL','DPT','GUST','HGT','PRES',
                          'RHUM','SPFH','TCDC','TMP','UGRD','VGRD',
                          'VIS','WDIR','WIND'),
    },
    ...
    'grid': {
        ...
        'subdir_by_hours':BY_HOURS_DIRPATH,
        'subdir_by_year':BY_YEAR_DIRPATH,
        'source':'acis',
        'sources':'RTMA/URMA/NDFD',
        'timezone':'UTC',
        'variable_map':{ # maps common variable names to reanalysis variables
            'apcp':'PCPN','dewpt':'DPT','pressure':'PRES',
            'rh':'RHUM','temp':'TMP'
        },
        'variable_names':('CEIL','DPT','GUST','HGT','PCPN','PRES',
                          'RHUM','SPFH','TCDC','TMP','UGRD','VGRD',
                          'VIS','WDIR','WIND'),
    },
}
...
CONFIG.sources.reanalysis.grid_file_maps = {
'fcast_obs': { # deprecated
      'data':'%(target_date)s-%(num_hours)shour-Data.h5',
      'APCP':'%(target_date)s-%(num_hours)shour-Precipitation.h5',
      'CEIL':'%(target_date)s-%(num_hours)shour-Cloud-Ceiling.h5',
      'DPT' :'%(target_date)s-%(num_hours)shour-Dewpoint.h5',
      'GUST':'%(target_date)s-%(num_hours)shour-Wind-Gust.h5',
      'HGT' :'%(target_date)s-%(num_hours)shour-Geopotential-Height.h5',
      'PCPN':'%(target_date)s-%(num_hours)shour-Precipitation.h5',
      'PRES':'%(target_date)s-%(num_hours)shour-Surface-Pressure.h5',
      'RHUM':'%(target_date)s-%(num_hours)shour-Relative-Humidity.h5',
      'SPFH':'%(target_date)s-%(num_hours)shour-Specific-Humidity.h5',
      'TCDC':'%(target_date)s-%(num_hours)shour-Total-Cloud-Cover.h5',
      'TMP' :'%(target_date)s-%(num_hours)shour-Temperature.h5',
      'UGRD':'%(target_date)s-%(num_hours)shour-U-Wind,h5',
      'VGRD':'%(target_date)s-%(num_hours)shour-V-Wind,h5',
      'VIS' :'%(target_date)s-%(num_hours)shour-Visibility,h5',
      'WDIR':'%(target_date)s-%(num_hours)shour-Wind-Direction,h5',
      'WIND':'%(target_date)s-%(num_hours)shour-Wind-Speed,h5',
      },
